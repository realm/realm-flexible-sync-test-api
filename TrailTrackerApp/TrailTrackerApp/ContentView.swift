            ...
            case .open(let realm):
                HomeView()
                    .onAppear(perform: {
                        Task {
                            do {
                                let users: QueryResults<Profile> = try await realm.subscriptions.subscribe(to: { $0.identifierId == app.currentUser!.id })
                                if users.first == nil {
                                    try realm.write {
                                        let profile = Profile()
...
struct ProfileView: View {
    @ObservedRealmObject var user: Profile
    @Environment(\.realm) var realm
    @State var favouriteResults: QueryResults<Trail>?
    @State var wantsToDoResults: QueryResults<Trail>?
    @State var doneResults: QueryResults<Trail>?
    ...
    var body: some View {
        ...
        .onAppear(perform: {
            Task {
                do {
                    let (favouriteTrails, wantsToDoTrails, doneTrails) = try await realm.subscriptions.subscribe(to:
                                                                                                                    QuerySubscription<Trail> { $0._id.in(user.favourites) },
                                                                                                                 QuerySubscription<Trail> { $0._id.in(user.wantsToDo) },
                                                                                                                 QuerySubscription<Trail> { $0._id.in(user.done) })
                    self.favouriteResults = favouriteTrails
                    self.wantsToDoResults = wantsToDoTrails
                    self.doneResults = doneTrails
                } catch {
                    showErrorMessage = true
                    errorMessage = error.localizedDescription
                }
